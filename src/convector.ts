...
import { Car, CarController } from "car-cc";
...
export const CarControllerBackEnd = ClientFactory(CarController, adapter);
...
export async function InitServerIdentity() {
    await initAdapter;
    const res = await CarControllerBackEnd.query("2");
    try {
        const serverIdentity = new Car(res).toJSON();
